...
class UserForm extends Component {
  ...
  initialState = () => {
    Logger.log('debug', `UserForm.initialState()`);
  ...
  formDefaults = () => {
    Logger.log('debug', `UserForm.formDefaults()`);
  ...
  submitData = async () => {
    ...
    let payload = {
      'is_verified': this.state['is_verified'] ? true : false,
      'profile': {}
    };
    for (const input of Object.keys(this.props.data)) {
      if (input in this.state) {
        if (['first_name', 'last_name', 'joined_at'].includes(input)) {
          payload['profile'][input] = this.state[input];
        }
  ...
  handleSubmit = async (evt) => {
    Logger.log('debug', `UserForm.handleSubmit(###)`);
    evt.preventDefault();
  ...
  render() {
    ...
    return (
      <Translation>
        {
          (t) => 
            <div className="form-user" ref={this.formTop}>
                ...
                <Row>
                  ...
                  <Col md="9">
                    ...
                    <Card>
                      ...
                      <CardBody>
                        ...
                        <FormGroup>
                          <Label for="usernameInput">{t('user_username')}</Label>
                          <Input 
                            type="text"
                        ...
                        <FormGroup>
                          <Label for="emailInput">{t('user_email')}</Label>
                          <Input 
                            type="text"
                    ...
                    <Card>
                      <CardHeader>
                        <strong><i className="icon-user pr-1"></i> {t('user_account_header_profile')} </strong>
                      </CardHeader>
                      <CardBody>
                    
                        <FormGroup>
                          <Label for="first_nameInput">{t('user_first_name')}</Label>
                          <Input 
                            type="first_name"
                        ...
                        <FormGroup>
                          <Label for="last_nameInput">{t('user_last_name')}</Label>
                          <Input 
                            type="last_name"
                        ...
                        <FormGroup>
                          <Label for="joined_atInput">{t('user_joined_at')}</Label>
                          <Input 
                            type="text"
                  ...
                  <Col md="3">
                    ...
                    <FormMetadata
                      ...
                      onDeleteRedirectTo={'UsersScreen'}
  ...
  componentDidMount() {
    Logger.log('silly', `UserForm.componentDidMount()`);
  ...
  componentDidUpdate() {
    Logger.log('silly', `UserForm.componentDidUpdate()`);
  }

  componentWillUnmount() {
    Logger.log('silly', `UserForm.componentWillUnmount()`);
