...
export function loadAppKeys(page=1, limit=10, order=null, cb=function(){}) {
  ...
  return async function(dispatch) {
    ...
    if (200 === response.get('status')) {
      ...
      Logger.log('info', `Get API app keys list success. Page: ${page}, Limit: ${limit}, Order: ${order}.`);

      const normalizedEntities = normalize(response.getIn(['data', 'app_keys']), [schema.appKey]);
      const data = {
    ...
    // get app keys list failure
      ...
      Logger.log('info', `Get API app keys list failure. Page: ${page}, Limit: ${limit}, Order: ${order}.`);
...
export function loadAppKey(id, cb=function(){}) {
  ...
  return async function(dispatch) {
    ...
    if (200 === response.get('status')) {
      ...
      const normalizedEntities = normalize([response.getIn(['data', 'app_key'])], [schema.appKey]);
      const data = {
